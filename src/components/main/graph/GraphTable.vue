...
<div class="content-wrapper" style="min-height:960px;">
      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">新建资源池 <b>{{Title}}</b></h3>
            <button class="btn btn-success pull-right" data-toggle="modal" data-target="#create">新建字段</button>
            <button class="btn btn-success pull-right" data-toggle="modal" data-target="#map" style="margin-right:10px">字段映射</button>
          </div>
  ...
  <div class="modal fade" id="map" tabindex="-1" role="dialog" aria-labelledby="createLabel">
    <div class="modal-dialog" role="document">
      <div class="modal-content" style="width: 400px;margin:0 auto">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
          <h4 class="modal-title" id="createLabel">字段映射</h4>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label>标准字段</label>
          ...
          <div class="form-group">
            ...
            <select class="form-control" v-model="mode">
              ...
              <option value="GRNN">GRNN</option>
            </select>
          </div>
          <div class="form-group">
            <label>映射结果</label>
            <input class="form-control" type="text" v-model="result"/>
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal" @click="close">关闭</button>
          <button type="button" class="btn btn-primary" @click="map">映射</button>
        </div>
      </div>
    </div>
  </div>
